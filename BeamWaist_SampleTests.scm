;This code is to test the effect of the lenses on the focal length of the beam
; for a simplified setup that only consists on the lenses and quartz bar in a
; straight line.
...
(define zPosEnd 1700.00)

; (define NameDown " ")
; (define NameUp " ")
(define TestScreen "TestScreen")
(define rootPathSurvey "C:/Users/KMFin/OneDrive/Desktop/Grad School/Laser Orientation/Survey/BeamWaistTest1/BeamWaistSampleTestsBar")
...
(define test-beam-waist
	(lambda (screenInc screenSeparation yStartFirst yEndFirst yInc)
		(cond 
			( (< yStartFirst yEndFirst) 'done)
			( else
				(set! angleTest 0)
				(screen-run angleTest screenInc screenSeparation yStartFirst)
				(move-lens "+250_Lens" -38 (- yStartFirst yInc) 60.538)
				(move-lens "-250_Lens" -38 (- (- yStartFirst screenSeparation) yInc) 60.538)
				(define screenName (string-append "TestScreen_" (number->string angleTest) ))
				(delete screenName)
				(test-beam-waist screenInc screenSeparation (- yStartFirst yInc) yEndFirst yInc)
			)
		)
	)
)
...
(define beam-run
	(lambda (screenInc screenSeparation yStartFirst yEndFirst yInc)
		(run-nominal -30)
		(test-beam-waist screenInc screenSeparation yStartFirst yEndFirst yInc)
